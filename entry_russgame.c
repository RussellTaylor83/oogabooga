int entry(int argc, char **argv)
{
	...
	while (!window.should_close)
	{
		...
		draw_frame.view = m4_make_scale(v3(4, 4, 1.0));
		...
		player_pos = v2_add(player_pos, v2_mulf(input_axis, 1000.0 * delta_t));

		Vector2 player_size = v2(409, 281);

		Matrix4 xform = m4_scalar(1.0);
		xform = m4_translate(xform, v3(player_pos.x, player_pos.y, 0));
		xform = m4_translate(xform, v3(player_size.x * -0.5, 0, 0));

		draw_image_xform(player, xform, player_size, COLOR_GREEN);
